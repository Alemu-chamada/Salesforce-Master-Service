...
SOQL_QUERIES: dict[str, str] = {
    ...
    "Opportunity": "SELECT Id, AccountId, Name, StageName, Amount, CloseDate, Probability, Type, LeadSource, OwnerId, CreatedDate, LastModifiedDate, (SELECT Id, OpportunityId, Product2Id, ProductCode, Name, Quantity, UnitPrice, TotalPrice, ServiceDate FROM OpportunityLineItems), (SELECT Id, ContactId, Role, IsPrimary FROM OpportunityContactRoles) FROM Opportunity",
    ...
    "Case": "SELECT Id, AccountId, ContactId, CaseNumber, Subject, Description, Status, Priority, Origin, Type, Reason, OwnerId, CreatedDate, LastModifiedDate, (SELECT Id, CommentBody, CreatedDate, CreatedById FROM CaseComments) FROM Case",
    ...
    "Campaign": "SELECT Id, Name, Type, Status, StartDate, EndDate, IsActive, NumberOfLeads, NumberOfContacts, NumberOfResponses, NumberOfWonOpportunities, AmountAllOpportunities, OwnerId, CreatedDate, LastModifiedDate, (SELECT Id, LeadId, ContactId, Status, HasResponded, FirstRespondedDate FROM CampaignMembers) FROM Campaign",
